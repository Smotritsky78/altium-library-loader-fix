...
Sub CreateCourtyardLine(x1,y1,x2,y2,width)
    ...
    ' Disabled for AD 26.1.1+: PCBServer.SendMessageToRobots footprint.I_ObjectAddress,c_Broadcast,PCBM_BoardRegisteration,atrack.I_ObjectAddress
    PCBServer.PostProcess
End Sub
...
Sub CreateCourtyardCircle(x,y,rad,width)
    ...
    ' Disabled for AD 26.1.1+: PCBServer.SendMessageToRobots footprint.I_ObjectAddress,c_Broadcast,PCBM_BoardRegisteration,acircle.I_ObjectAddress
    PCBServer.PostProcess
End Sub
...
Sub CreateSilkscreenArc(x,y,rad,startAngle,endAngle,width)
    ...
    ' Disabled for AD 26.1.1+: PCBServer.SendMessageToRobots footprint.I_ObjectAddress,c_Broadcast,PCBM_BoardRegisteration,aarc.I_ObjectAddress
    PCBServer.PostProcess
End Sub
...
Sub CreateRoundedPTH(x,y,length,width,holesize,name,plated,rotation)
    ...
    ' Disabled for AD 26.1.1+: PCBServer.SendMessageToRobots footprint.I_ObjectAddress,c_Broadcast,PCBM_BoardRegisteration,apad.I_ObjectAddress
    PCBServer.PostProcess
End Sub
...
Sub ExtractEPW(fldr, dst)
   ...
   ' Make sure destination folder exists. ZIP mode uses InstalledDir\Temp\.
   If Not fso.FolderExists(dst) Then
      fso.CreateFolder(dst)
   End If

   For Each f In fldr.Items
      ' Do not use f.Type here: it is localized in non-English Windows
      ' (for example Russian Windows does not return "File folder" / "EPW File").
      If f.IsFolder Then
         ExtractEPW f.GetFolder, dst
      ElseIf LCase(fso.GetExtensionName(f.Name)) = "epw" Then
         cbFilename = f.Name
         sa.NameSpace(dst).CopyHere f.Path, 16

         ' CopyHere is asynchronous for ZIP namespaces, so wait until the file appears.
         t = 0
         Do While Not fso.FileExists(dst & cbFilename) And t < 100
